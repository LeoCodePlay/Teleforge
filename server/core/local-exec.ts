...
function truncate(text: string, chr: number): string {
  if (!text || text.length <= chr) return text || '';
  const keep = Math.floor(chr * 0.6), tail = chr - keep;
  return text.slice(0, keep) + '\n…[输出过长,已截断,省略 ' + (text.length - chr) + ' 字符]…\n' + text.slice(text.length - tail);
}

export function execLocal(command: string, { cwd, timeout = LOCAL_EXEC.DEFAULT_TIMEOUT_MS, maxOutput = LOCAL_EXEC.MAX_OUTPUT_CHARS }: ExecLocalOptions = {}): Promise<ExecLocalResult> {
