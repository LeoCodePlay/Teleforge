...
1. 确认 Teleforge 服务正在运行(`npm run dev` / 桌面端启动均可,默认 `http://127.0.0.1:4000`)。
2. 点扩展图标打开 popup。
3. 填服务端地址(默认已是 `http://127.0.0.1:4000`),点 **获取配对并连接**。
   - 扩展会向 `/api/browser-bridge/pair` 取一份配对 token,然后连上 `ws://127.0.0.1:4000/ws/ext`。
   - 状态点变绿 = 已连接。
