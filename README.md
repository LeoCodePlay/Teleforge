...
- **内置终端**：真实 PTY 交互式终端（/ws/term），配合**命令台**手动执行远程命令，实时输出与退出码；运行中的命令可点「⏹ 停止」或直接按 **Ctrl+C** 终止（先发 SIGINT，兜底强杀）。
- **文件传输**：文件管理器「⬆ 文件 / ⬆ 文件夹」把本地文件/目录树上传到**当前目录**（带进度条，上传后自动刷新）；右键「⬇ 下载」文件直接下载、文件夹流式打包成 tar.gz 下载；文件查看器也提供「⬇ 下载」。
- **技能系统**：内置技能库，Agent 可通过工具按需加载 `SKILL.md` 指令；技能面板可查看/搜索/新建/编辑，内置技能可"复制到"任意级别成为可编辑副本。
